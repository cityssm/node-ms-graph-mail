...
await describe('MsGraphMailApi', async () => {
  await it.skip('should list mail folders', async () => {
    ...
    assert.ok(
      ...
      'Expected mailFolders to contain at least one folder'
    )
  })
  ...
  await it.skip('should get inbox folder', async () => {
    ...
    assert.strictEqual(
      inboxFolder.displayName.toLowerCase(),
      'inbox',
      'Expected inboxFolder displayName to include "Inbox"'
    )
  })
  ...
  await it.skip('should list messages with attachments in inbox folder', async () => {
    ...
    assert.ok(
      messages.every((message) => message.hasAttachments === true),
      'Expected all messages to have attachments'
    )
  })
  ...
  await it.skip('should list attachments for a message in inbox folder', async () => {
    ...
    const attachments = await api.listMessageAttachments(
      messageWithAttachments.id
    )
    ...
    for (const attachment of attachments) {
      await fs.writeFile(
        path.join('test', 'data', attachment.name),
        Buffer.from(attachment.contentBytes, 'base64')
      )

      console.log(
        `Saved attachment "${attachment.name}" to test/data directory`
      )
    }
  })
  ...
  await it.skip('should move a message to the archive folder', async () => {
    ...
    assert.strictEqual(
      movedMessage.parentFolderId,
      archiveFolder.id,
      'Expected moved message parentFolderId to match archive folder ID'
    )
  })
