...
const buildQuery = OdataQuery as unknown as typeof OdataQuery.default
...
export default class MsGraphMailApi {
  readonly #apiUrlRoot: string
  readonly #client: Client
  readonly #clientCredential: ClientSecretCredential
   ...
   */
  async archiveMessage(messageId: string): Promise<MsGraphMailMessage> {
   ...
   * @throws {Error} Will throw an error if the Archive folder is not found or if the API request fails.
   */
  async getArchiveFolder(
    options?: Pick<MsGraphMailApiOptions<MsGraphMailFolder>, 'select'>
  ): Promise<MsGraphMailFolder> {
    const folder = await this.getMailFolderByDisplayName(
      wellKnownFolderNames.Archive,
      options
    )
  ...
  async getMailFolderByDisplayName(
    ...
    if (
      folderOptions.select !== undefined &&
   ...
   */
  async getOutboxFolder(
    options?: Pick<MsGraphMailApiOptions<MsGraphMailFolder>, 'select'>
  ): Promise<MsGraphMailFolder> {
    const folder = await this.getMailFolderByDisplayName(
      wellKnownFolderNames.Outbox,
      options
    )
   ...
   */
  async getSentItemsFolder(
    options?: Pick<MsGraphMailApiOptions<MsGraphMailFolder>, 'select'>
  ): Promise<MsGraphMailFolder> {
    const folder = await this.getMailFolderByDisplayName(
      wellKnownFolderNames.SentItems,
      options
    )
  ...
  async listMessages(
    ...
    return (await this.#callGetApi(
      `/mailFolders/${folderIdOrWellKnownFolderName}/messages`,
      options
    )) as MsGraphMailMessage[]
  }
  ...
  async markMessageAsRead(messageId: string): Promise<void> {
    await this.#client
      .api(`${this.#apiUrlRoot}/messages/${messageId}`)
      .patch({ isRead: true })
  }
  ...
  async moveMessage(
    ...
    return response as MsGraphMailMessage
  }
   ...
   * @throws {Error} Will throw an error if the API request fails or if the response is not in the expected format.
   */
  async sendMessage(message: MsGraphMailSendableMessage): Promise<void> {
    await this.#client.api(`${this.#apiUrlRoot}/sendMail`).post({ message })
  }

  async #callGetApi<T>(
    endpoint: string,
    options: MsGraphMailApiOptions<T> = {}
  ): Promise<unknown> {
    ...
    const response = (await api.get()) as { value: T[] }
